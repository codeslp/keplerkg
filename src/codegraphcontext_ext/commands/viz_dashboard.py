...
_CATEGORY_PRINCIPLES: dict[str, str] = {
    "architecture": "Module interfaces should be deep enough to create leverage for callers and locality for maintainers.",
    ...
    "coupling": "Keep dependencies directional and module seams explicit so change stays local.",
...
_RULE_PRINCIPLES: dict[str, str] = {
    ...
    "cross_file_private_access": "Respect module seams and keep private implementation details private.",
    ...
    "shallow_pass_through_function": "A module should earn its interface by concentrating behavior that would otherwise leak into callers.",
    "single_adapter_seam": "A seam is real when behavior varies across adapters; one adapter is only hypothetical indirection.",
    "suggest_better_name": "Prefer names that align with the established vocabulary of similar code.",
    "test_reaches_private_seam": "Tests should cross the same interface as callers instead of reaching into private implementation seams.",
    ...
    "unreferenced_public_class": "Unused public interfaces create maintenance burden and should not survive by accident.",
    "unreferenced_public_function": "Unused public interfaces create maintenance burden and should not survive by accident.",
    "wide_public_function_interface": "A wide public interface lowers module depth by making callers learn too many entry points.",
}
...
def _rule_principle(rule_id: str, category: str) -> str:
    ...
    return _RULE_PRINCIPLES.get(rule_id) or _CATEGORY_PRINCIPLES.get(
        category,
        "Each rule protects a code-quality seam so behavior stays understandable, maintainable, and safe to change.",
    )
